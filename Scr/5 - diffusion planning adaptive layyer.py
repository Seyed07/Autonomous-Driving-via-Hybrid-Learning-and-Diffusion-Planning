...
import numpy as np  # اگر قبلاً import نشده، اضافه کن
...
class LearnableAdaptiveMask(nn.Module):
    def __init__(self, image_height=IMAGE_HEIGHT_cnn, image_width=IMAGE_WIDTH_cnn, 
                 init_speed_scale=0.5, init_lidar_scale=0.5, 
                 base_lower_weight=1.0, base_upper_weight=0.0):
        super().__init__()
        self.image_height = image_height
        self.image_width = image_width
        self.base_lower_weight = base_lower_weight
        self.base_upper_weight = base_upper_weight
        
        # ضرایب قابل آموزش
        self.speed_scale = nn.Parameter(torch.tensor(init_speed_scale, dtype=torch.float32))
        self.lidar_scale = nn.Parameter(torch.tensor(init_lidar_scale, dtype=torch.float32))
        
    def forward(self, speed, lidar_min, lidar_safe_dist=MIN_SAFE_DISTANCE, max_speed=MAX_SPEED):
        """
        تولید ماسک فضایی تطبیقی با ضرایب قابل یادگیری.
        
        Args:
            speed: tensor [B] یا [B,1] سرعت فعلی
            lidar_min: tensor [B] یا [B,1] حداقل فاصله LiDAR
            lidar_safe_dist: float, فاصله ایمن LiDAR
            max_speed: float, حداکثر سرعت
        
        Returns:
            mask_channel: tensor [B, H, W, 1] ماسک نرمال‌شده
        """
        B = speed.shape[0] if speed.dim() > 0 else 1
        
        # نرمال‌سازی ورودی‌ها
        speed = speed.view(B) if speed.dim() > 1 else speed
        lidar_min = lidar_min.view(B) if lidar_min.dim() > 1 else lidar_min
        
        # نرمال‌سازی سرعت
        speed_norm = torch.clamp(speed / max_speed, 0.0, 1.0)
        # سطح خطر LiDAR
        hazard_level = torch.clamp((lidar_safe_dist - lidar_min) / lidar_safe_dist, 0.0, 1.0)
        
        # ضرایب پویا
        speed_factor = 1.0 + self.speed_scale * speed_norm
        hazard_factor = 1.0 + self.lidar_scale * hazard_level
        
        lower_mult = self.base_lower_weight * speed_factor * hazard_factor
        upper_mult = torch.tensor(self.base_upper_weight, dtype=torch.float32, device=speed.device)
        
        # ساخت ماسک گرادیان
        mask_np = np.zeros((self.image_height, self.image_width), dtype=np.float32)
        for y in range(self.image_height):
            rel_pos = y / (self.image_height - 1)
            mask_np[y, :] = upper_mult.item() + (lower_mult.mean().item() - upper_mult.item()) * rel_pos
        
        mask_torch = torch.tensor(mask_np, dtype=torch.float32, device=speed.device)
        mask_torch /= mask_torch.max() + 1e-6
        
        mask_channel = mask_torch.unsqueeze(0).unsqueeze(-1).repeat(B, 1, 1, 1)  # [B, H, W, 1]
        return mask_channel
...
class AutonomousDrivingEnv(Env):
    ...
    def __init__(self, car_def_name="MY_ROBOT"):  # Initialize with default Webots node name for the vehicle
        ...
        self.observation_space = spaces.Dict({
            "image": spaces.Box(0, 1, (IMAGE_HEIGHT_cnn, IMAGE_WIDTH_cnn, 4), dtype=np.float32),  # # Define observation space for normalized RGBa image
            "lidar": spaces.Box(0, 100, (LIDAR_MATRIX_HEIGHT,), dtype=np.float32)  # Define observation space for LiDAR ranges capped at 100m
        })
        self.learnable_mask = LearnableAdaptiveMask(
        image_height=IMAGE_HEIGHT_cnn,
        image_width=IMAGE_WIDTH_cnn,
        init_speed_scale=0.5,
        init_lidar_scale=0.5
         ).to(self.device)
    ...
    def _get_state(self):
        """
        دریافت مشاهدات: تصویر تغییراندازه‌شده/نرمال‌شده + داده‌های LiDAR + کانال ماسک تطبیقی.
        """
        image_data_cnn = np.zeros((IMAGE_HEIGHT_cnn, IMAGE_WIDTH_cnn, 3), dtype=np.float32)  # تصویر RGB
        lidar_data = np.full(LIDAR_MATRIX_HEIGHT, 100.0, dtype=np.float32)  # داده‌های LiDAR پیش‌فرض
        if self.camera:
            img_buf = self.camera.getImage()
            if img_buf:
                img = np.frombuffer(img_buf, dtype=np.uint8).reshape((IMAGE_HEIGHT, IMAGE_WIDTH, 4))  # دریافت تصویر
                img_rgb = cv2.cvtColor(img, cv2.COLOR_RGBA2RGB)  # تبدیل به RGB
                img_resized = cv2.resize(img_rgb, (IMAGE_WIDTH_cnn, IMAGE_HEIGHT_cnn), interpolation=cv2.INTER_AREA)  # تغییر اندازه
                image_data_cnn = img_resized.astype(np.float32) / 255.0  # نرمال‌سازی
        if self.lidar:
            raw_lidar = self.lidar.getRangeImage()
            if raw_lidar:
                lidar_data = np.array(raw_lidar, dtype=np.float32)  # دریافت داده‌های LiDAR
                lidar_data[np.isinf(lidar_data)] = 100.0  # محدود کردن مقادیر بی‌نهایت

        # تبدیل داده‌ها به تنسور
        device = self.device
        speed_tensor = torch.tensor([self.algo_speed], dtype=torch.float32, device=device)  # سرعت فعلی
        lidar_min_tensor = torch.tensor([np.min(lidar_data)], dtype=torch.float32, device=device)  # حداقل فاصله LiDAR
        
        # تولید ماسک با ماژول قابل یادگیری
        with torch.no_grad():  # بدون گرادیان، چون اینجا فقط برای مشاهدات است
            mask_channel = self.learnable_mask(speed_tensor, lidar_min_tensor)  # [1, H, W, 1]
        
        # تبدیل تصویر RGB به تنسور و ترکیب با ماسک
        rgb_tensor = torch.tensor(image_data_cnn, dtype=torch.float32, device=device)  # [H, W, 3]
        rgb_tensor = rgb_tensor.unsqueeze(0)  # [1, H, W, 3]
        obs_with_mask = torch.cat([rgb_tensor, mask_channel], dim=-1)  # [1, H, W, 4]
        
        # تبدیل به آرایه NumPy برای سازگاری با Gym
        obs_with_mask_np = obs_with_mask.squeeze(0).cpu().numpy()  # [H, W, 4]
        
        return {"image": obs_with_mask_np, "lidar": lidar_data}  # دیکشنری مشاهدات
    ...
    def safe_features_extraction(self, features_extractor, observations):
        """
        استخراج ایمن ویژگی‌ها با مدیریت تبدیل شکل تنسورها.
        پشتیبانی از ورودی ۴ کاناله (RGB + Mask).
        """
        try:
            img = observations["image"]
            # مدیریت ابعاد دسته‌ای و کانال برای ۴ کانال
            if img.dim() == 4:  # حالت دسته‌ای: [B, H, W, 4]
                if img.shape[3] == 4:
                    img = img.permute(0, 3, 1, 2)  # به [B, 4, H, W]
                elif img.shape[1] == 4:
                    pass
                else:
                    raise ValueError(f"Expected 4 channels, got invalid. Shape: {img.shape}")
            elif img.dim() == 3:  # نمونه تکی: [H, W, 4]
                if img.shape[2] == 4:
                    img = img.permute(2, 0, 1).unsqueeze(0)  # به [1, 4, H, W]
                else:
                    raise ValueError(f"Expected single sample with 4 channels, got invalid. Shape: {img.shape}")
            
            # بررسی نهایی
            if img.shape[1] != 4:
                raise ValueError(f"Expected 4 channels after permute, got {img.shape[1]}. Shape: {img.shape}")
            
            # مدیریت LiDAR
            lidar_input = observations["lidar"]
            if isinstance(lidar_input, np.ndarray):
                if len(lidar_input.shape) == 1:
                    lidar_tensor = torch.from_numpy(lidar_input).float().unsqueeze(0)  # [1, LIDAR_HEIGHT]
                else:
                    lidar_tensor = torch.from_numpy(lidar_input).float()  # [B, LIDAR_HEIGHT]
            else:
                if lidar_input.dim() == 1:
                    lidar_tensor = lidar_input.unsqueeze(0)  # [1, LIDAR_HEIGHT]
                else:
                    lidar_tensor = lidar_input
            
            # هماهنگ‌سازی اندازه دسته
            B_img = img.shape[0]
            B_lidar = lidar_tensor.shape[0]
            if B_img != B_lidar:
                print(f"[DEBUG-SAFE] Batch mismatch: img={B_img}, lidar={B_lidar}. Syncing...")
                if B_lidar == 1 and B_img > 1:
                    lidar_tensor = lidar_tensor.repeat(B_img, 1)
                elif B_img == 1 and B_lidar > 1:
                    img = img.repeat(B_lidar, 1, 1, 1)
                else:
                    raise ValueError(f"Cannot sync batches: img={B_img}, lidar={B_lidar}")
            
            safe_observations = {"image": img.to(self.device), "lidar": lidar_tensor.to(self.device)}
...
class CustomCNNWithLiDAR(BaseFeaturesExtractor):
    """
    استخراج‌کننده ویژگی‌های سفارشی برای SB3: CNN برای تصویر (۴ کانال: RGB + Mask) + MLP برای LiDAR.
    """
    def __init__(self, observation_space: gym.spaces.Dict, features_dim: int = 512):
        super(CustomCNNWithLiDAR, self).__init__(observation_space, features_dim)
        self.image_shape = observation_space.spaces["image"].shape  # (64, 64, 4)
        self.lidar_shape = observation_space.spaces["lidar"].shape
        self.H = self.image_shape[0]  # 64
        self.W = self.image_shape[1]  # 64
        self.C = self.image_shape[2]  # 4
        
        self.cnn = nn.Sequential(
            nn.Conv2d(4, 32, kernel_size=5, stride=2, padding=2),  # ورودی ۴ کاناله
            nn.ReLU(),
            nn.Conv2d(32, 64, kernel_size=3, stride=2, padding=1),
            nn.ReLU(),
            nn.Conv2d(64, 64, kernel_size=3, stride=2, padding=1),
            nn.ReLU(),
            nn.Flatten(start_dim=1),  # Flatten به [B, features]
        )
        
        with torch.no_grad():
            sample_input = torch.zeros(1, self.C, self.H, self.W)  # ورودی نمونه ۴ کاناله
    ...
    def forward(self, observations):
        img = observations["image"]  # [B, H, W, 4] یا [H, W, 4]
        lidar = observations["lidar"]  # [B, LIDAR_HEIGHT] یا [LIDAR_HEIGHT]
        ...
        if img.dim() == 4:
            if img.shape[3] == 4:
                ...
                raise ValueError(f"Unexpected batch shape: {img.shape}")
        elif img.dim() == 3:
            if img.shape[2] == 4:
                img = img.permute(2, 0, 1).unsqueeze(0)  # به [1, 4, H, W]
            else:
                raise ValueError(f"Unexpected single shape: {img.shape}")
        
        # بررسی نهایی: باید [B, 4, H, W] باشد
        if img.shape[1] != 4:
            raise ValueError(f"Expected 4 channels, got {img.shape[1]}. Shape: {img.shape}")
        
        # پردازش تصویر
        img_features = self.cnn(img)  # [B, cnn_output_dim]
        img_flat = img_features.view(img_features.size(0), -1)  # [B, features]
        
        # پردازش LiDAR
        if lidar.dim() == 1:
            lidar = lidar.unsqueeze(0)  # [1, LIDAR_HEIGHT]
        lidar_features = self.lidar_mlp(lidar)  # [B, 32]
        lidar_flat = lidar_features.view(lidar_features.size(0), -1)  # [B, 32]
        
        # ترکیب ویژگی‌ها
        if img_flat.dim() != 2 or lidar_flat.dim() != 2:
            raise ValueError(f"Shape mismatch: img={img_flat.shape}, lidar={lidar_flat.shape}")
        if img_flat.shape[0] != lidar_flat.shape[0]:
            raise ValueError(f"Batch size mismatch: img={img_flat.shape[0]}, lidar={lidar_flat.shape[0]}")
        
        combined = torch.cat([img_flat, lidar_flat], dim=1)  # [B, cnn_dim + 32]
...
class ImitationLearningCallback(BaseCallback):
    ...
    # اضافه کردن لاگ ضرایب ماسک
        if hasattr(self.env, 'learnable_mask'):
            speed_scale = self.env.learnable_mask.speed_scale.item()
            lidar_scale = self.env.learnable_mask.lidar_scale.item()
            print(f"[PLOT-MASK] Learned coefficients | speed_scale: {speed_scale:.4f}, lidar_scale: {lidar_scale:.4f}")
