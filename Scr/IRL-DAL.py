...
class LearnableAdaptiveMask(nn.Module):
    ...
    def forward(self, speed, lidar_min, lidar_safe_dist=MIN_SAFE_DISTANCE, max_speed=MAX_SPEED):
        """
        Adaptive spatial mask generation with learnable coefficients.
        Args:
        speed: tensor [B] or [B, 1], current speed
        lidar_min: tensor [B] or [B, 1], minimum LiDAR distance
        lidar_safe_dist: float, LiDAR safe distance
        max_speed: float, maximum speed
        Returns:
        mask_channel: tensor [B, H, W, 1], normalized mask
        """
...
class GuidanceEnergy(nn.Module):
    ...
    def forward(self, x, cond):
        ...
        lidar_min_target = cond.get('lidar_min_target', torch.full((x.size(0),), 1.5, device=x.device)) 
...
class AutonomousDrivingEnv(Env):
    ...
    def set_model(self, model):
        ...
        lam_params = list(self.learnable_mask.parameters())
        policy_params = list(self.model.policy.parameters())
        all_params = policy_params + lam_params
    ...
    def _get_state(self):
        ...
        if self.camera:
            ...
            if img_buf:
                ...
                img_resized = cv2.resize(img_rgb, (IMAGE_WIDTH_cnn, IMAGE_HEIGHT_cnn), interpolation=cv2.INTER_AREA) 
        ...
        device = self.device
        speed_tensor = torch.tensor([self.algo_speed], dtype=torch.float32, device=device)
        lidar_min_tensor = torch.tensor([np.min(lidar_data)], dtype=torch.float32, device=device) 
        ...
        return {"image": obs_with_mask_np, "lidar": lidar_data}  
